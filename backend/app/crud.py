from . import schemas
from passlib.context import CryptContext

# Hashing setup remains the same
pwd_context = CryptContext(schemes=["bcrypt"], deprecated="auto")

def get_password_hash(password):
    """Hashes a plain-text password."""
    return pwd_context.hash(password)

# --- User CRUD Functions ---

def create_user(db_conn, user: schemas.UserCreate):
    """Creates a new user in the MySQL database."""
    hashed_password = get_password_hash(user.password)
    # The `dictionary=True` argument makes the cursor return results as dictionaries
    with db_conn.cursor(dictionary=True) as cur:
        cur.execute(
            "INSERT INTO users (username, email, hashed_password) VALUES (%s, %s, %s);",
            (user.username, user.email, hashed_password)
        )
        # MySQL doesn't have `RETURNING *`, so we get the last inserted ID
        # and then fetch that user to return the complete object.
        new_user_id = cur.lastrowid
        db_conn.commit()
        cur.execute("SELECT * FROM users WHERE user_id = %s;", (new_user_id,))
        new_user = cur.fetchone()
        return new_user

def get_user_by_email(db_conn, email: str):
    """Retrieves a user by their email address."""
    with db_conn.cursor(dictionary=True) as cur:
        cur.execute("SELECT * FROM users WHERE email = %s;", (email,))
        user = cur.fetchone()
        return user

def get_user_by_id(db_conn, user_id: int):
    """Retrieves a user by their ID."""
    with db_conn.cursor(dictionary=True) as cur:
        cur.execute("SELECT * FROM users WHERE user_id = %s;", (user_id,))
        user = cur.fetchone()
        return user

# --- Post CRUD Functions ---

def create_post(db_conn, post: schemas.PostCreate, user_id: int):
    """Creates a new post."""
    with db_conn.cursor(dictionary=True) as cur:
        cur.execute(
            "INSERT INTO posts (title, content, user_id) VALUES (%s, %s, %s);",
            (post.title, post.content, user_id)
        )
        new_post_id = cur.lastrowid
        db_conn.commit()
        # Fetch the newly created post
        cur.execute("SELECT * FROM posts WHERE post_id = %s;", (new_post_id,))
        new_post = cur.fetchone()
        return new_post

def get_posts(db_conn, skip: int = 0, limit: int = 10):
    """
    Retrieves a list of posts with owner details using a standard JOIN,
    and calculates like/view counts using subqueries.
    """
    with db_conn.cursor(dictionary=True) as cur:
        sql_query = """
            SELECT
                p.post_id, p.title, p.content, p.created_at, p.user_id,
                u.username, u.email as user_email, u.created_at as user_created_at,
                (SELECT COUNT(*) FROM post_likes pl WHERE pl.post_id = p.post_id) as likes_count,
                (SELECT COUNT(*) FROM post_views pv WHERE pv.post_id = p.post_id) as views_count
            FROM posts p
            JOIN users u ON p.user_id = u.user_id
            ORDER BY p.created_at DESC
            LIMIT %s OFFSET %s;
        """
        cur.execute(sql_query, (limit, skip))
        posts = cur.fetchall()
        return posts

# --- Comment CRUD Functions ---
def create_comment(db_conn, comment: schemas.CommentCreate, post_id: int, user_id: int):
    """Creates a new comment on a post."""
    with db_conn.cursor(dictionary=True) as cur:
        cur.execute(
            "INSERT INTO comments (content, post_id, user_id) VALUES (%s, %s, %s);",
            (comment.content, post_id, user_id)
        )
        new_comment_id = cur.lastrowid
        db_conn.commit()
        cur.execute("SELECT * FROM comments WHERE comment_id = %s;", (new_comment_id,))
        return cur.fetchone()

# --- Post Like CRUD Functions ---

def create_post_like(db_conn, post_id: int, user_id: int):
    """Likes a post for a user. Ignores duplicates."""
    with db_conn.cursor(dictionary=True) as cur:
        # Using INSERT IGNORE is a MySQL-specific way to handle conflicts.
        cur.execute(
            "INSERT IGNORE INTO post_likes (post_id, user_id) VALUES (%s, %s);",
            (post_id, user_id)
        )
        db_conn.commit()
    return {"status": "ok"}

def delete_post_like(db_conn, post_id: int, user_id: int):
    """Unlikes a post for a user."""
    with db_conn.cursor(dictionary=True) as cur:
        cur.execute(
            "DELETE FROM post_likes WHERE post_id = %s AND user_id = %s;",
            (post_id, user_id)
        )
        db_conn.commit()
    return {"status": "ok"}

# --- Follow CRUD Functions ---
def create_follow(db_conn, follower_id: int, followed_id: int):
    """Creates a follow relationship."""
    with db_conn.cursor(dictionary=True) as cur:
        cur.execute(
            "INSERT IGNORE INTO follows (follower_id, followed_id) VALUES (%s, %s);",
            (follower_id, followed_id)
        )
        db_conn.commit()
    return {"status": "ok"}

def delete_follow(db_conn, follower_id: int, followed_id: int):
    """Deletes a follow relationship."""
    with db_conn.cursor(dictionary=True) as cur:
        cur.execute(
            "DELETE FROM follows WHERE follower_id = %s AND followed_id = %s;",
            (follower_id, followed_id)
        )
        db_conn.commit()
    return {"status": "ok"}

def create_collection(db_conn, collection: schemas.CollectionCreate, user_id: int):
    """Creates a new collection for a user."""
    with db_conn.cursor(dictionary=True) as cur:
        cur.execute(
            "INSERT IGNORE INTO collections (name, user_id) VALUES (%s, %s);",
            (collection.name, user_id)
        )
        new_collection_id = cur.lastrowid
        db_conn.commit()
        cur.execute("SELECT * FROM collections WHERE collection_id = %s;", (new_collection_id,))
        return cur.fetchone()

def create_bookmark(db_conn, bookmark: schemas.BookmarkCreate, user_id: int):
    """Creates a bookmark, linking a user, post, and collection."""
    with db_conn.cursor(dictionary=True) as cur:
        # Check if the collection belongs to the user for security
        cur.execute(
            "SELECT user_id FROM collections WHERE collection_id = %s;",
            (bookmark.collection_id,)
        )
        collection_owner = cur.fetchone()
        if not collection_owner or collection_owner['user_id'] != user_id:
            return None # Or raise an exception for better error handling

        cur.execute(
            "INSERT IGNORE INTO bookmarks (user_id, post_id, collection_id) VALUES (%s, %s, %s);",
            (user_id, bookmark.post_id, bookmark.collection_id)
        )
        db_conn.commit()
        return {"status": "ok", "user_id": user_id, "post_id": bookmark.post_id, "collection_id": bookmark.collection_id}